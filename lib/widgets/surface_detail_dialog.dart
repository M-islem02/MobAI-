...
class _SurfaceDetailDialogState extends State<SurfaceDetailDialog>
  ...
  Widget build(BuildContext context) {
    ...
    final dialogW = (screenW * 0.75).clamp(600.0, 1100.0);
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${zone.type.label} • ${zone.widthM}m × ${zone.heightM}m • ${zone.areaM2.toStringAsFixed(1)}m² • Section: ${zone.section.isEmpty ? "—" : zone.section}',
                  style: const TextStyle(
          ...
          if (widget.onEdit != null)
            IconButton(
              ...
              icon: const Icon(Icons.edit_rounded, color: AppColors.textMid),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 10),
          _posChip('Area', '${zone.areaM2.toStringAsFixed(1)}m²',
              Icons.square_foot),
        ],
      ),
    );
  ...
  Widget _buildRackViewTab(StorageZone zone) {
    ...
    final cols = max(1, (zone.widthM / 1.0).floor());
    final rows = max(1, (zone.heightM / 1.0).floor());
        ...
        children: [
          Row(
            children: [
              ...
              Text(
                isRack
                    ? 'Rack Layout — ${cols}×$rows slots (1m² each)'
...
class _ZoneDetailPainter extends CustomPainter {
  ...
  _ZoneDetailPainter({required this.zone, this.animValue = 0});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int r = 0; r < rows; r++) {
      for (int c = 0; c < cols; c++) {
        final x = ox + c * cellS;
        final y = oy + r * cellS;
        final rect = Rect.fromLTWH(x + 1, y + 1, cellS - 2, cellS - 2);
        ...
        // Slot label
        if (cellS > 20) {
          final label = '${String.fromCharCode(65 + (r % 26))}${c + 1}';
          _paintText(
            canvas,
            label,
            Offset(x + cellS / 2 - label.length * 3, y + cellS / 2 - 5),
            (cellS * 0.22).clamp(6.0, 11.0),
            Colors.black54,
          );
        }
      }
    ...
    for (int r = 0; r < rows; r++) {
      _paintText(canvas, '${String.fromCharCode(65 + (r % 26))}',
          Offset(ox - 16, oy + r * cellS + cellS / 2 - 5), 10,
          const Color(0xFF455A64), bold: true);
    }
    // Col labels
    for (int c = 0; c < cols; c++) {
      _paintText(canvas, '${c + 1}',
          Offset(ox + c * cellS + cellS / 2 - 4, oy - 16), 10,
          const Color(0xFF455A64), bold: true);
    }
  }
